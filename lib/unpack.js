...
var unpack = function(input, objs, custom) {
  ...
  if (byte < 0x80) {
    ...
    switch (byte) {
      ...
      case 0xae: return; // @todo
      case 0xaf: return; // @todo
    }
  } else if (byte < 0xc0) {
    var len = byte === 0xbf ? input.readVarint() : byte - 0xb0,
        ret = {};
    ...
    var len = byte !== 0xcf ? byte - 0xc0 : input.readVarint(),
        ret = [];
    ...
    var len = byte !== 0xef ? byte - 0xd0 : input.readVarint();
